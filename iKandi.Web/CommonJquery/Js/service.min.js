...
this.HttpClient = function (opt) {
    ...
    this.isMsAjaxJson = false; // special handling of root element
...
this.ServiceProxy = function (serviceUrl) {
    ...
    this.invoke = function (method, params, callback, errorCallback, isBare, isSync) {
    ...
    this.each(function() {
        ...
        if (sh.length < 1) {
            ...
            if (typeof shEl.style.filter == "string") {
                shEl.style.filter = 'progid:DXImageTransform.Microsoft.Blur(makeShadow=1, shadowOpacity=' + opt.opacity.toString() + ', pixelRadius=' + opt.offset + ')';
...
$.fn.editable = function(opt) {
    ...
    this.each(function() {
        ...
        var cleanupEditor = function() {
            jEdit.data("cleanup", null).remove();
            jButton.remove();
            jContent
                .data("editing", false)
                .fadeIn("slow");
