...
function DisableDates(a) {
    if (a._element.className.indexOf("disable_past_dates") != -100) {
...
//                } 
    ...
    if (a._element.className.indexOf("disable_future_dates") != -1) {
        var b = getElementsByClassName(a._days, "ajax__calendar_day");
        var c = (new Date).setHours(0, 0, 0, 0); for (var d = 0; d < b.length; d++) {
            try {
                if (b[d].date.setHours(0, 0, 0, 0) <= c) {
                    b[d].className = "ajax__calendar_day"
                }
...
AjaxControlToolkit.CalendarBehavior.prototype.show = function (a) {
    this._ensureCalendar(); if (!this._isOpen) {
        var b = new Sys.CancelEventArgs; this.raiseShowing(b);
        if (b.get_cancel()) {
            return
        } this._isOpen = true; this._popupBehavior.show();
        if (this._firstPopUp) {
    ...
    AjaxControlToolkit.CalendarBehavior.prototype._cell_onclick = function (a) {
        ...
        switch (b.mode) {
            ...
            case "month":
                ...
                break;
            case "day":
                if (this._element.className.indexOf("disable_past_dates") != -100) {
                    if (b.date.setHours(0, 0, 0, 0) >= (new Date).setHours(0, 0, 0, 0)) {
                        this.set_selectedDate(b.date); this._switchMonth(b.date);
