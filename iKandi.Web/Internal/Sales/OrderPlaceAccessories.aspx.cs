...
{
    public partial class OrderPlaceAccessories : System.Web.UI.Page
    {
        public int OrderID
        {
            get
            ...
            for (int AccNo = 0; AccNo < dlstAccessoriesPopup.Items.Count; AccNo++)
            {
                ...
                if (SeqId == Convert.ToInt16(hdnSeqId.Value))
                {
                    if (hdnId.Value != "")
                    {
                        ...
                        {
                            int UserId = ApplicationHelper.LoggedInUser.UserData.UserID;
                            bool iDelete = objOrderPlaceController.Delete_Accessories_OrderPlace(Convert.ToInt64(hdnId.Value), UserId);
                        }
                    }
                }
                else
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            bool save = objOrderPlaceController.Insert_Update_Accessories(objAccessCollection, OrderID, -1, UserId);
            if (save == true)
            {
