<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CostingSheet.aspx.cs" MasterPageFile="~/layout/Secure.Master"
    Inherits="iKandi.Web.CostingSheet" EnableEventValidation="false" %>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            if (tabLICollection.length > 0)
                $(tabLICollection[0]).css('background', '#F9DDF4');
            ...
            function (objStyle) {
                ...
                if (null != objStyle && objStyle.StyleID != -1) {
                    ...
                    if (costingId == -1) {
                        ...
                        proxy.invoke('CloneStyleNumber', { parentStyleNumber: parentStyleNumber, styleNumber: styleNumber, clientId: -1, departmentId: -1, costingId: costingId, orderIDs: orderIDs, selectedItemSample: selectedItemSample, avg: Average_val, Cad: CAD_val, obsheet: OBSheet_val, sam: SAM_val, ob: OB_val, cmt: CMT_val },
                    function (success) {
        ...
        function ShowCostingSheet(sender, styleId, styleNumber) {
            tabLICollection.css('background', '#FFFFFF');
            $(sender).css('background', '#F9DDF4');
            ...
            if (activeCostingSheet.length == 1) {
                ...
                activeCostingSheet.height(activeCostingSheet.contents().document.body).find('div').attr('scrollHeight');
        ...
        function RefreshPage(styleNumber) {
            ...
            window.location = "costingSheet.aspx?sn=" + styleNumber;
        }
    ...
    <div id="tabs_costing">
        ...
        <asp:HiddenField ID="hdnIsUcknowledge" runat="server" />
            <asp:Repeater ID="repeaterCostingTabs" runat="server">
                <ItemTemplate>
                    <li class="costing-tab-li" id='<%# "tab-" + Eval("StyleID").ToString() %>' styleid='<%# Eval("StyleID") %>' stylenumber='<%# Eval("StyleNumber") %>'>
                        <div>
            ...
            <li id="addButton" runat="server"><span onclick="ShowAddStyleNumberBox()" class="add-style-number">
                +</span> </li>
        </ul>
    </div>
    <asp:Repeater ID="repeaterCostingSheets" runat="server">
        <ItemTemplate>
            <div id='<%# Eval("StyleID") %>' style="display: none; height:1800x;" class="costing-sheet">
                <iframe id="ifCosting" runat="server" height="1800" src='<%# "TabCostingSheet.aspx?cid=" + Convert.ToInt32(Eval("CostingID")) +"&StyleID="+ Convert.ToInt32(Eval("StyleID")) +"&ClientID="+ Convert.ToInt32(Eval("ClientID")) +"&DepartmentID="+ Convert.ToInt32(Eval("DepartmentID")) +"&IsUcknowledge=" + hdnIsUcknowledge.Value  %>'
                    width="100%" class="costing-iframe" frameborder="0" scrolling="no"
                    stylenumber='<%# Eval("StyleNumber") %>'></iframe>
            </div>
        </ItemTemplate>
    </asp:Repeater>
    ...
    <div class="style_number_box">
       ...
       <asp:ListItem Text="With Sample" Value="2"></asp:ListItem>
            ...
            <tr>
                ...
                <td>
                    <asp:TextBox runat="server" CssClass="do-not-allow-typing" ID="txtStyleNumber1" Width="60px"
                        MaxLength="4"></asp:TextBox>
                    -
                    <asp:TextBox runat="server" ID="txtStyleNumber2" Width="60px" MaxLength="2" onKeyPress="return VinCheck(event);" onpaste="return false;">
                    </asp:TextBox>
