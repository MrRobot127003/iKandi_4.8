<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RemainFinishingQty.aspx.cs" Inherits="iKandi.Web.Internal.Merchandising.RemainFinishingQty" %>
...
<body bgcolor="#FFFFFF">
  ...
  <div id="frmHed" style="padding: 5px 0px; background-color: #405D99; color: #FFFFFF;
    font-size: 22px; font-family: Arial; text-transform: none; text-align: center">Reshuffle Remain Finishing Quantity</div>
  <br />
        ...
        <asp:GridView ID="gvReAllocation" runat="server" AutoGenerateColumns="false" Width="98%" HeaderStyle-Height="35px" OnRowDataBound="gvReAllocation_RowDataBound" CssClass="item_list">
          <Columns>
            ...
            <asp:TemplateField HeaderText="Unit Qty" ItemStyle-Width="20%">
              <ItemTemplate>
                <asp:HiddenField ID="hdnTotalUnAssignFinishQty" runat="server" Value='<%#Eval("TotalUnAssignFinishQty") %>' />
                <asp:Label ID="lblUnitQty" runat="server" Text='<%# string.Format("{0:#,#0}",Eval("FinishingShare")) %>' Visible="True"></asp:Label>
              </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Finish Qty" ItemStyle-Width="20%">
              <ItemTemplate>
                <asp:TextBox ID="txtFinishQty" runat="server" Text='<%# string.Format("{0:#,#0}",Eval("FinishQty")) %>' ReadOnly="true" Visible="True"></asp:TextBox>
              </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Remain Finish Qty" ItemStyle-Width="20%">
              <ItemTemplate>
                <asp:TextBox ID="txtRemainFinishQty" runat="server" Text='<%# string.Format("{0:#,#0}",Eval("RemainFinishQty")) %>' ReadOnly="true" Visible="True"></asp:TextBox>
              </ItemTemplate>
            </asp:TemplateField>
