...
{
    public partial class FabricDebitNote : System.Web.UI.Page
    {
        ...
        {
            Session["EmailDebitID"] = null;
            Session["EmailPoID"] = null;
            ...
            if (!IsPostBack)
            {
                ...
                //txtCGST.Text = Convert.ToDecimal(dtCmt.Rows[0]["CGST"].ToString()).ToString();
                //txtIGST.Text = Convert.ToDecimal(dtCmt.Rows[0]["IGST"].ToString()).ToString();
                //txtSGST.Text = Convert.ToDecimal(dtCmt.Rows[0]["SGST"].ToString()).ToString();
                Session["DebitNoteParticularsList"] = null;
                BindData(0, PartyBillNumber);
                CalculateGrandTotal();
                // Session["DebitNoteParticularsList"] = null;
        ...
        private void BindBillDropdownList(string PartyBillNumber)
        {
            ...
            if (Accessory_Srv_BillList.Count > 0)
            {
                ...
                ddlBillNo.DataValueField = "PartyBillNumber";
                ddlBillNo.DataBind();

                //foreach (var item in Accessory_Srv_BillList)
                //{
                //    foreach (System.Web.UI.WebControls.ListItem listItem in ddlBillNo.Items)
                //    {
                //        if (item.Sequence == 0 && listItem.Text.ToLower() == item.BillDetails.ToLower())
                //        {
                //            listItem.Attributes.Add("class", "dropDownListColor");
                //        }                        
                //    }
                //}

                if (PartyBillNumber != "" && PartyBillNumber != null)
                {
                    ddlBillNo.SelectedValue = PartyBillNumber.ToString();
                }
        ...
        //        string[] sChar = BillDetails.Spl//        hdnBillAmount.Value = sAmount.Trim();it('(');
        //        string[] sChar1 = sChar[1].Split(')');
        //        string sAmount = sChar1[0].ToString();
        //        hdnBillAmount.Value = sAmount.Trim();
        //    }
        ...
        private void BindData(int IsPageRefresh, string PartyBillNumber)
        {


            FabricDebitNoteCls objAccessoryDebitNote = objFabricWorking.Get_FabricDebitNote(SupplierPoId, DebitNoteId, PartyBillNumber, Srv_id);
            ...
            if (objAccessoryDebitNote.QtyCheckedBy > 0)
            {
                ...
                foreach (var user in iKandi.Web.Components.ApplicationHelper.Users)
                {
                    if (objAccessoryDebitNote.QtyCheckedBy == user.UserID)
                    {
                        ...
                        rblMail.SelectedValue = "0";
                        DivMail.Attributes.Add("style", "display:block;font-weight: 600;");
                    }
                }
                IsSignatureDone = 1;
            }
            ...
            {
                DivMail.Attributes.Add("style", "display:none;font-weight: 600;");
                lblCheckedDate.Text = "";
            }

            DataTable dt = new DataTable();
            dt.Clear();
            dt.Columns.Add("DebitNoteId");
            dt.Columns.Add("DebitNoteParticularId");
            dt.Columns.Add("ParticularName");
            dt.Columns.Add("DebitQuantity");
            dt.Columns.Add("DebitRate");
            dt.Columns.Add("IsExtraQty");
            dt.Columns.Add("Fab_DebitNote_SRVID");
            dt.Columns.Add("SRVID");
            dt.Columns.Add("PartyBillNumber");
            dt.Columns.Add("Amount");
            DataRow Dt_Row = dt.NewRow();
            Dt_Row["DebitNoteId"] = -1;
            Dt_Row["DebitNoteParticularId"] = -1;
            Dt_Row["ParticularName"] = "";
            Dt_Row["DebitQuantity"] = 0;//changed 0 to -1
            Dt_Row["DebitRate"] = 0;//changed 0 to -1 
            Dt_Row["IsExtraQty"] = -1;
            Dt_Row["Fab_DebitNote_SRVID"] = -1; //changed from -1 to (0)
            Dt_Row["SRVID"] = Srv_id;
            Dt_Row["PartyBillNumber"] = -1;
            Dt_Row["Amount"] = 0;
            dt.Rows.Add(Dt_Row);
            List<FabricDebitNoteParticulars> objDebitNoteParticulars = objAccessoryDebitNote.FabricDebitNoteParticularsList;
            if (objDebitNoteParticulars.Count > 0)
            {
            ...
            {
                grdAccessoryDebitNot.DataSource = dt;
                grdAccessoryDebitNot.DataBind();
            }
            ...
            if (DebitNoteId > 0)
            {
                ...
                BindBillDropdownList(objAccessoryDebitNote.PartyBillNumber);
        ...
        protected void grdAccessoryDebitNot_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            ////        TextBox txtDebitParticur = (TextBox)grdAccessoryDebitNot.Rows[AccNo].FindControl("txtDebitParticur");
            ////        TextBox txtDebitQty = (TextBox)grdAccessoryDebitNot.Rows[AccNo].FindControl("txtDebitQty");
            ////        TextBox txtDebitRate = (TextBox)grdAccessoryDebitNot.Rows[AccNo].FindControl("txtDebitRate");
            ////        HiddenField hdnAmount = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdnAmount");
        ...
        protected void grdAccessoryDebitNot_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            HiddenField hdnsrvid2 = (HiddenField)row.FindControl("hdnsrvid2");
            Label lblsrvid = (Label)row.FindControl("lblSrvID");
            HiddenField hdnPartyBillNumber = (HiddenField)row.FindControl("hdnPartyBillNumber");
            Label lbleditSrvid = (Label)row.FindControl("lbleditSrvid");
            if (hdnsrvid2.Value != "" && lbleditSrvid.Text != "" && hdnPartyBillNumber.Value != null)
            {
                lbleditSrvid.Text = hdnPartyBillNumber.Value;
            }
            ...
            for (int AccNo = 0; AccNo < grdAccessoryDebitNot.Rows.Count; AccNo++)
            {
                ...
                HiddenField hdnsrvid3 = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdnsrvid2");
                HiddenField hdnPartyBillNumber2 = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdnPartyBillNumber");
                if (hdnId != null && lblDebitRate != null)
                {
                    objDebitNoteParticulars.DebitNoteParticularId = hdnId.Value == "" ? 0 : Convert.ToInt32(hdnId.Value);
                    objDebitNoteParticulars.ParticularName = lblDebitParticur.Text;
                    ...
                    objDebitNoteParticulars.Amount = hdnAmount.Value == "" ? 0 : Convert.ToDouble(hdnAmount.Value);
                    objDebitNoteParticulars.IsExtraQty = hdntypes.Value == "" ? -1 : Convert.ToInt32(hdntypes.Value);
                    objDebitNoteParticulars.Fab_DebitNote_SRVID = hdnsrvid.Value == "" ? -1 : Convert.ToInt32(hdnsrvid.Value);
                    objDebitNoteParticulars.SRVID = hdnsrvid3.Value;
                    objDebitNoteParticulars.PartyBillNumber = hdnPartyBillNumber2.Value;
        ...
        protected void grdAccessoryDebitNot_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Insert")
            {
                ...
                for (int AccNo = 0; AccNo < grdAccessoryDebitNot.Rows.Count; AccNo++)
                {
                    ...
                    HiddenField hdntypes = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdntypes");
                    HiddenField hdnsrvid = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdnsrvid");
                    HiddenField hdnsrvid2 = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdnsrvid2");
                    HiddenField hdnPartyBillNumber = (HiddenField)grdAccessoryDebitNot.Rows[AccNo].FindControl("hdnPartyBillNumber");
                    ...
                    objDebitNoteParticulars.SRVID = hdnsrvid2.Value;
                    objDebitNoteParticulars.PartyBillNumber = hdnPartyBillNumber.Value;
                    MaxId = MaxId + AccNo + 1;
        ...
        protected void grdAccessoryDebitNot_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                e.Row.Cells[3].Text = "Reason";
            }
            if ((e.Row.RowState == DataControlRowState.Edit || e.Row.RowState == (DataControlRowState.Edit | DataControlRowState.Alternate)) && e.Row.RowType == DataControlRowType.DataRow)
            {
            ...
            else
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lblsrvid = (Label)e.Row.FindControl("lblSrvID");
                    HiddenField hdnPartyBillNumber = (HiddenField)e.Row.FindControl("hdnPartyBillNumber");
                    HiddenField hdnsrvid2 = (HiddenField)e.Row.FindControl("hdnsrvid2");

                    if ((hdnsrvid2.Value.Trim() != "" && lblsrvid.Text.Trim() != null) || hdnPartyBillNumber.Value.Trim() != "")
                    {
                        lblsrvid.Text = hdnPartyBillNumber.Value == "-1" ? "" : hdnPartyBillNumber.Value;
                    }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (TotalAmount > 0)
            {
                ...
                objAccessoryDebitNote.PartyBillNumber = ddlBillNo.SelectedValue;
                ...
                if (chkQtyCheckedBy.Checked == true)
                {
                ...
                int UserId = iKandi.Web.Components.ApplicationHelper.LoggedInUser.UserData.UserID;
                if (divSignature2.Visible == false)
                {
                    int iSave = objFabricWorking.Save_Accessory_DebitNote(objAccessoryDebitNote, UserId);
                    if (DebitNoteId <= 0)
                    {
                        DebitNoteId = objAccessoryDebitNote.DebitNoteId;
                    }
                }
                //if (iSave > 0)
                //{

                if (rblMail.SelectedValue == "1")
                {
        ...
        protected void ddlBillNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            PartyBillNumber = ddlBillNo.SelectedItem.Value;

            BindData(1, PartyBillNumber);
            CalculateGrandTotal();
