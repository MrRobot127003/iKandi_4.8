...
{
    public partial class CourierDispatchMailScheduler : System.Web.UI.Page
    {
        NotificationController objcontroller = new NotificationController();
        CourierController controller = new CourierController();

        string MailType = "Courier Dispatch List";
        //string MailType = "BIPL Core Figures";
        string Flag = string.Empty;
        string FitsPath = string.Empty;
        DateTime CourierDate;
        string name = "";
        protected void Page_Load(object sender, EventArgs e)
        {
            string url = Constants.MainUrlMail + "/CourierDispatchList.aspx";
            string EmailContent = HttpContent(url);

            DataTable result = new DataTable();
            result = objcontroller.CourierDispatchListExists(DateTime.Today);
            int res = Convert.ToInt32(result.Rows[0]["result"].ToString());
            if (res == 1)
            {
        ...
        public Boolean SendClientRegistrationEmail(String ClientName, String UsernamePasswordList, String ToEmail, string MailType)
        {
            ...
            try
            {
                ...
                {
                    if (MailType == "Courier Dispatch List")
                    {
                        List<Attachment> atts = new List<Attachment>();

                        DataTable courierDate_dt = new DataTable();
                        courierDate_dt = objcontroller.GetCourierDispatchListDate(DateTime.Today);
                        CourierDate = Convert.ToDateTime(courierDate_dt.Rows[0]["CourierDate"].ToString());                        
                        //string pdfFilePathIkandi = Path.Combine(Constants.TEMP_FOLDER_PATH, "Courier List-IKANDI " + CourierDate.ToString("dd MMM yyy") + ".pdf");
                        //bool success = controller.GenerateDailyCourierReport(pdfFilePathIkandi, CourierDate, 1);
                        name = "Courier List-IKANDI " + CourierDate.ToString("dd MMM yyy") + ".pdf";
                        if (File.Exists(Constants.TEMP_FOLDER_PATH + name))
                        {

                            FitsPath = Path.Combine(Constants.TEMP_FOLDER_PATH, name);
        ...
        public Boolean SendEmail(String FromEmail, List<String> To, List<String> CC, List<String> BCC, String Subject, String Content, List<Attachment> Attachments, Boolean hasAppendAttachment, Boolean isAsync)
        {
            ...
            mailMessage.Subject = "COURIER DISPATCH LIST FOR " + CourierDate.ToString("dd MMM yyy");
