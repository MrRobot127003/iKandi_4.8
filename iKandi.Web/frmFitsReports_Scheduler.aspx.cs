...
{
    public partial class frmFitsReports_Scheduler : System.Web.UI.Page
    {
        string MailType = "Merchandising & Fabric Performance Report";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Request.QueryString["Type"].ToString() != "" || Request.QueryString["Type"] != null)
                {
                    ...
                    {
                        //string url = Constants.MainUrlMail + "/FitsReport.aspx?Type=FitsSampling";
                        Server.Transfer("FitsReport.aspx?Type=FitsSampling");
                        //Response.Redirect(url);
                        //string EmailContent = HttpContent(url);
                    }
                    else if (Type == "Restexcels")
                    {
                        //string url = Constants.MainUrlMail + "/FitsReport.aspx?Type=Restexcels";
                        //Response.Redirect(url);
                        Server.Transfer("FitsReport.aspx?Type=Restexcels");
                        //string EmailContent = HttpContent(url);
                    }
                    else if (Type == "BindReport")
                    {
                        string url = Constants.MainUrlMail + "/FitsReport.aspx?Type=BindReport";
                        string EmailContent = HttpContent(url);
                        SendClientRegistrationEmail("test", "kumar", EmailContent, MailType);
                    }
                }

                //string url = Constants.MainUrlMail + "/FitsReport.aspx";
                //string EmailContent = HttpContent(url);
                //SendClientRegistrationEmail("test", "kumar", EmailContent, MailType);
            }
            catch (Exception ex)
        ...
        public Boolean SendClientRegistrationEmail(String ClientName, String UsernamePasswordList, String ToEmail, string MailType)
        {
            ...
            try
            {
                ...
                {
                    if (MailType == "Merchandising & Fabric Performance Report")
                    {
                        List<Attachment> atts = new List<Attachment>();

                        //ReportType = "HandOver-PreOrder";
                        name = "FITS and Sampling Reports" + ".xlsx";
                        ...
                        name = "Pattern_Sample_Pending" + ".xlsx";
                        if (File.Exists(Constants.FITS_FOLDER_PATH + name))
                        {
                        ...
                        name = "BuyerPOPendingReports" + ".xlsx";
                        if (File.Exists(Constants.FITS_FOLDER_PATH + name))
                        {
                        ...
                        name = "Pending_Weight_Style_Reports" + ".xlsx";
                        if (File.Exists(Constants.FITS_FOLDER_PATH + name))
                        {
                        ...
                        name = "FabriInhouseShortforUpcomingDC" + ".xlsx";
                        if (File.Exists(Constants.FITS_FOLDER_PATH + name))
                        {
                        ...
                        name = "AM_and_Material_Performance" + ".xlsx";
                        if (File.Exists(Constants.FITS_FOLDER_PATH + name))
                        {
                        ...
                        name = "Pending_Cost_Confirmation" + ".xlsx";
                        if (File.Exists(Constants.FITS_FOLDER_PATH + name))
                        {
