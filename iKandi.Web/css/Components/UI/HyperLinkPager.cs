...
namespace iKandi.Components.UI
{
  /// <summary>
  ...
  public class HyperLinkPager : Label, INamingContainer
  {
    ...
    {
        //int PageAdd;
        if (Math.Ceiling(Convert.ToDecimal(this.TotalRecords) / this.PageSize) > this.PageIndex)
            writer.Write(string.Format(this.CurrentPageStringFormat, (this.PageIndex + 1), this.CalculateTotalPages(), this.TotalRecords));
        else
            writer.Write(string.Format(this.CurrentPageStringFormat, (this.PageIndex), this.CalculateTotalPages(), this.TotalRecords));
    ...
    void AddPageLinks()
    {
      ...
      for (int i = 0; i < pagingHyperLinks.Length; i++)
      {
        ...
        pagingHyperLinks[i].NavigateUrl = CreatePagerURL((i).ToString());
    ...
    public bool ResetPageIndex = false;
    int _pageIndex = 0;
    ...
    protected virtual string CreatePagerURL(string pageIndex)
    {
      ...
      {
        url = context.Request.Url.AbsoluteUri.ToString() + "?PageIndex=" + pageIndex;
      }
      else
      {
        ...
        {
          url = Regex.Replace(context.Request.Url.AbsoluteUri.ToString(), @"PageIndex=(\d+\.?\d*|\.\d+)", "PageIndex=" + pageIndex);
        }
      }

      // Create a new PostBackOptions object and set its properties.
      PostBackOptions postBackOptions = new PostBackOptions(this);
      postBackOptions.ActionUrl = url;
      postBackOptions.AutoPostBack = false;
    ...
    public virtual int CalculateTotalPages()
    {
      ...
      totalPagesAvailable = Convert.ToInt32(Math.Ceiling(Convert.ToDecimal(TotalRecords) / PageSize));
